...
|---|---|---|---|
| **watr/optimize** | **20,401 B** | **93 ms** in-process | 119 KB min (36 KB gz) |
| `wasm-opt -Oz` | 19,852 B | 990 ms CLI | ~1.1 MB js / native binary |
| `wasm-opt -O3` | 22,302 B | — | — |

Smaller than `-O3` on every module; ties or beats `-Oz` on 14 of 20 (within 2.8% overall — the rest is instruction-selection-grade rewrites).
Binaryen's native core outruns watr on multi-MB single modules (a 5.5 MB module: 0.3 s vs 2.2 s, sizes within 4%) —
