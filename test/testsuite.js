...
const hasExnRef = (() => {
  try {
    // Node v22 V8 treats exnref (0x69) as non-nullable (ref exn), so ref.null exn
    // (nullable) fails as an argument where exnref is expected. Node v24+ treats
    // exnref as (ref null exn) (nullable), accepting ref.null exn.
    // (module (func $f (param exnref) (result exnref) (local.get 0))
    //         (func (result exnref) (call $f (ref.null exn))))
    new WebAssembly.Module(new Uint8Array([0x00,0x61,0x73,0x6d,0x01,0x00,0x00,0x00,0x01,0x0a,0x02,0x60,0x01,0x69,0x01,0x69,0x60,0x00,0x01,0x69,0x03,0x03,0x02,0x00,0x01,0x0a,0x0d,0x02,0x04,0x00,0x20,0x00,0x0b,0x06,0x00,0xd0,0x69,0x10,0x00,0x0b]))
    return true
  } catch { return false }
})()
