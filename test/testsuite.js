...
const hasExceptionHandling = (() => {
  try {
    // Test try_table with catch - the strictest exnref test
    // (module (tag $e) (func (try_table (catch $e 0))))
...
const hasExnRef = (() => {
  try {
    // Node v22 V8: catch_ref provides nullable exnref, failing a block expecting (ref exn).
    // Node v24+: catch_ref provides non-nullable (ref exn), satisfying the block.
    // (module (tag $e) (func (result (ref exn))
    //   (block $h (result (ref exn)) (try_table (catch_ref $e $h) (unreachable)) (unreachable))))
    new WebAssembly.Module(new Uint8Array([0x00,0x61,0x73,0x6d,0x01,0x00,0x00,0x00,0x01,0x09,0x02,0x60,0x00,0x00,0x60,0x00,0x01,0x64,0x69,0x03,0x02,0x01,0x01,0x0d,0x03,0x01,0x00,0x00,0x0a,0x11,0x01,0x0f,0x00,0x02,0x64,0x69,0x1f,0x40,0x01,0x01,0x00,0x00,0x00,0x0b,0x00,0x0b,0x0b]))
    return true
  } catch { return false }
})()
...
t.mute('/test/official/instance.wast', async function () { await file(this.name, { spectest }) })
...
t.skip('/test/official/proposals/custom-descriptors/br_on_cast_desc_eq.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/br_on_cast_desc_eq_fail.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/exact-casts.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/exact-func-import.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/array_new_exact.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/br_on_cast.wast', async function () { await file(this.name, { spectest }) })
t.skip('/test/official/proposals/custom-descriptors/br_on_cast_fail.wast', async function () { await file(this.name, { spectest }) })
