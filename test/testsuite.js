...
const hasExnRef = (() => {
  try {
    // Node v22 V8 bug: catch_ref with a tagged param produces (ref exn) instead of exnref,
    // failing branch[1] type check when block expects (result i32 exnref).
    // (module (type $t (func (param i32))) (tag $e (type $t))
    //   (func (result i32 exnref) (block $h (result i32 exnref)
    //     (try_table (result i32) (catch_ref $e $h) (i32.const 0)) (ref.null exn))))
    new WebAssembly.Module(new Uint8Array([0x00,0x61,0x73,0x6d,0x01,0x00,0x00,0x00,0x01,0x0a,0x02,0x60,0x01,0x7f,0x00,0x60,0x00,0x02,0x7f,0x69,0x03,0x02,0x01,0x01,0x0d,0x03,0x01,0x00,0x00,0x0a,0x12,0x01,0x10,0x00,0x02,0x01,0x1f,0x7f,0x01,0x01,0x00,0x00,0x41,0x00,0x0b,0xd0,0x69,0x0b,0x0b]))
    return true
  } catch { return false }
})()
